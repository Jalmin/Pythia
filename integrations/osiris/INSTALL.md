...
- `src/components/OsirisMap.tsx` — `nws-alerts` polygon source + `nws-fill`/`nws-outline`
  layers; a `spin` prop (rotate/smart); globe pitch set to 0 (no tilt).
- `src/components/LayerPanel.tsx` — added "Storm / Flood Zones" toggle; removed the SDK
